...
class DynamicModel(TorqueDynamics):
...
def main():
    ...
    with open(os.path.join(RESULTS_DIR, "Athlete14_2_arm_only.pkl"), "rb") as file:
    ...
    # qs = np.vstack((qs.T, np.zeros((6, 152))))
    #
    # x_init = InitialGuessList()
    # x_init.add("q", qs[:n0], interpolation=InterpolationType.EACH_FRAME, phase = 0)
    # x_init.add("q", qs[n0:], interpolation=InterpolationType.EACH_FRAME, phase = 1)
    #
    # u_init = InitialGuessList()
    # u_init.add("tau", [0] * 7, phase=0)
    # u_init.add("tau", [0] * 7, phase=1)

    print("prepare ocp")
    filename = "/models/Athlete_14_inverse2.bioMod"
    print("model : ", filename)
    #ocp = prepare_ocp(biorbd_model_path=CURRENT_DIR + filename, final_time=(1,3), n_shooting=n_shooting,min_time=0.02, max_time=4, weight=0.01, n_threads=1)
    ocp = prepare_ocp(biorbd_model_path=CURRENT_DIR + filename, final_time=(1,3), n_shooting=n_shooting,min_time=0.02, max_time=4, weight=0.01, n_threads=1, u_init=u_init, x_init=x_init)
    print("ocp ready")
    ...
    if save == True :
        ...
        with open(os.path.join(RESULTS_DIR, "Athlete14_2_thorax.pkl"), "wb") as file:
            data = {"q":qs,
                    "qdot": qdots,
                    "tau": taus,
                    }
