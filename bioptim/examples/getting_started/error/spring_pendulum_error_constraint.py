"""
Cet exemple porte sur un triple pendule de 1.5 m de long, placé en position ATR (appui tendu renversé, en gymnastique),
devant effectuer une rotation complète. Le pendule est suspendu à une barre modélisée par un ressort, et doit éviter 
toute collision avec une barre inférieure situé derrière. En diagonale, la distance entre la barre supérieure
et la barre inférieure est de 1.81 m, ce qui rend théoriquement la contrainte inutile. Pourtant, on observe que la fonction 
de coût (MINIMIZE_CONTROL) atteint une valeur plus faible avec la contrainte que sans.
"""
...
def prepare_ocp(
    ...
    constraint: bool
        If the constraints should be included 
    ode_solver: OdeSolverBase = OdeSolver.RK4()
    ...
    if constraint == True:
        constraint_list.add(ConstraintFcn.SUPERIMPOSE_MARKERS, node=Node.ALL, first_marker="LowerBarMarker", second_marker="marker_6", min_bound = 0.02, max_bound = np.inf, axes=Axis.Y)
...
def main():
    ...
    with open(os.path.join(RAC, "results/pendulum_data_100_shooting.pkl"), "rb") as file:
    ...
    constraint = False 
    ocp = prepare_ocp(biorbd_model_path=RAC + "error/model/triple_pendulum.bioMod", final_time=4, n_shooting=n_shooting,min_time=0.5, max_time=10, weight=0.0001, n_threads=1, u_init=u_init, x_init=x_init, ode_solver=OdeSolver.COLLOCATION(polynomial_degree=5, method="radau"), constraint=constraint)
    ...
    # --- Solve the ocp --- #
    solver = Solver.IPOPT(online_optim=OnlineOptim.DEFAULT, _linear_solver = "ma57", _tol = 1e-8)   
    # sol = ocp.solve(Solver.IPOPT(online_optim=OnlineOptim.DEFAULT, _linear_solver = "ma57"))
    sol = ocp.solve(solver)
    
    # --- If one wants to save the result 
    save = True     
    if save == True :
        ...
        save_path = "../../mnt/c/Users/emmam/Documents/GIT/bioptim/bioptim/examples/getting_started/error/"
        name = "test_constraint_" + str(constraint) + ".pkl"
