...
class DynamicModel(TorqueBiorbdModel):
    ...
    def dynamics(
        ...
        stiffness = 14160
        damping = 91
        tau[0] = -stiffness * q[0] + damping * qdot[0] #z
        tau[1] = -stiffness * q[1] + damping * qdot[1]  # y
...
def prepare_ocp(
    ...
    """
    bio_model = (DynamicModel(biorbd_model_path), DynamicModel(biorbd_model_path), DynamicModel(biorbd_model_path), DynamicModel(biorbd_model_path))
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=3, target=0, weight=5, phase=0)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=8, target=0, weight=5, phase=0)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=9, target=0, weight=1, phase=0)
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=3, target=0, weight=5, phase=1)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=8, target=0, weight=5, phase=1)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=9, target=0, weight=1, phase=1)
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=3, target=0, weight=5, phase=2)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=8, target=0, weight=5, phase=2)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=9, target=0, weight=1, phase=2)
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=3, target=0, weight=5, phase=3)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=8, target=0, weight=5, phase=3)
    objective_functions.add(ObjectiveFcn.Lagrange.TRACK_STATE, key="q", index=9, target=0, weight=1, phase=3)
    ...
    constraint_list.add(ConstraintFcn.SUPERIMPOSE_MARKERS, node=Node.ALL, first_marker="LowerBarMarker", second_marker="LastMarker", min_bound = 0.02, max_bound = np.inf, axes=Axis.Y, phase=1)
    constraint_list.add(ConstraintFcn.SUPERIMPOSE_MARKERS, node=Node.END, first_marker="LowerBarMarker",second_marker="LastMarker", axes=Axis.Z, phase=0, min_bound=0.02, max_bound=0.02)

    constraint_list.add(ConstraintFcn.SUPERIMPOSE_MARKERS, node=Node.ALL, first_marker="LowerBarMarker", second_marker="LastMarker", min_bound = 0.02, max_bound = np.inf, axes=Axis.Y, phase=3)
    constraint_list.add(ConstraintFcn.SUPERIMPOSE_MARKERS, node=Node.END, first_marker="LowerBarMarker", second_marker="LastMarker", axes=Axis.Z, phase=2, min_bound=0.02, max_bound=0.02)
    ...
    x_bounds.add("q", bounds=bio_model[1].bounds_from_ranges("q"), phase=3)
    x_bounds.add("qdot", bounds=bio_model[1].bounds_from_ranges("qdot"), phase=3)

    x_bounds[0]["q"][:, 0] = 0
    x_bounds[0]["qdot"][:, 0] = 0  # speeds start at 0

    x_bounds[1]["q"][:, -1] = 0
    x_bounds[1]["q"][2, -1] = 2* np.pi  # ends with hands 360° rotated

    x_bounds[3]["q"][:, -1] = 0
    x_bounds[3]["q"][2, -1] = 4 * np.pi
...
def main():
    ...
    n_shooting = (50, 100, 50, 100)

    with open(os.path.join(RESULTS_DIR, "athlete14_2soleils_complete_dynamic_base.pkl"), "rb") as file:
    ...
    x_init.add("q", qs[:, n0:(n1 + n0)], interpolation=InterpolationType.EACH_FRAME, phase=1)
    x_init.add("q", qs[:, (n1  + n0):(n1 + 2*n0)], interpolation=InterpolationType.EACH_FRAME, phase=2)
    x_init.add("q", qs[:, (n1 + 2*n0):], interpolation=InterpolationType.EACH_FRAME, phase=3)

    x_init.add("qdot", qdots[:, :n0], interpolation=InterpolationType.EACH_FRAME, phase=0)
    x_init.add("qdot", qdots[:, n0:(n1 + n0)], interpolation=InterpolationType.EACH_FRAME, phase=1)
    x_init.add("qdot", qdots[:, (n1 + n0):(n1 + 2*n0)], interpolation=InterpolationType.EACH_FRAME, phase=2)
    x_init.add("qdot", qdots[:, (n1 + 2*n0):], interpolation=InterpolationType.EACH_FRAME, phase=3)
    ...
    u_init.add("tau", taus[:,  n0 + n1 - 2:(2*n0 + n1 - 3)], interpolation=InterpolationType.EACH_FRAME, phase=2)
    u_init.add("tau", taus[:, (2*n0 + n1 - 3):], interpolation=InterpolationType.EACH_FRAME, phase=3)
    ...
    #ocp = prepare_ocp(biorbd_model_path=CURRENT_DIR + filename, final_time=(1, 3, 1, 3), n_shooting=n_shooting, min_time=0.02, max_time=2, weight=0.1, n_threads=32, use_sx=False)
    ocp = prepare_ocp(biorbd_model_path=CURRENT_DIR + filename, final_time=(1,3,1,3), n_shooting=n_shooting,min_time=0.02, max_time=2, weight=1, n_threads=32, u_init=u_init, x_init=x_init, use_sx=False)
    print("ocp ready")
    ...
    if save == True:
        ...
        with open(os.path.join(RESULTS_DIR, "athlete14_2soleils_complete_dynamic.pkl"), "wb") as file:
            data = {"q": qs,
                    "qdot": qdots,
                    "tau": taus,
                    }
